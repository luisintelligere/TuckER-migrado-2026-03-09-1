...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Tuning por asignatura con validacion temporal estricta.")
...
def load_seed_configs(top_seeds_per_metric: int) -> dict[str, list[dict]]:
    source_path = RESULTS_DIR / "tuning_temporal_todos.csv"
    if not source_path.exists():
        raise FileNotFoundError(f"No existe {source_path}; ejecuta primero tuning_temporal.py")
...
def main() -> None:
    args = parse_args()
    print("Cargando datos para tuning walk-forward...")
    ...
    all_path = RESULTS_DIR / "tuning_walkforward_todos.csv"
    best_val_path = RESULTS_DIR / "tuning_walkforward_mejores_validacion.csv"
    best_oot_path = RESULTS_DIR / "tuning_walkforward_mejores_2023.csv"
    summary_path = RESULTS_DIR / "tuning_walkforward_resumen.csv"
