...
# Versión que usa ENTITY RANKING (filtered setting) para validación y early stopping.
# Combina: warm start + early stopping + entity ranking (evaluación estándar de KG).
...
class Experiment:
    ...
    def evaluate(self, model, data):
        """
        Evaluación estándar de KG: entity ranking con filtered setting.
        Para cada (e1, r), rankea TODAS las entidades como posible tail.
        Filtra otros tails válidos conocidos (de train+valid+test) antes de calcular el rank.
        """
        model.eval()
        ...
        # Filtered setting: usar TODOS los datos conocidos
    ...
    def train_and_eval(self, init_embeddings_path=None, init_vocab_path=None):
        ...
        if init_embeddings_path and init_vocab_path:
            ...
            try:
                ...
                print(f"Error al cargar embeddings: {e}. Usando inicializacion aleatoria.")
                model.init()
        else:
            model.init()
        ...
        print(f"Se monitorearan los embeddings de los siguientes alumnos: {alumnos_muestra}")
        ...
        for it in range(1, self.num_iterations + 1):
            ...
            with torch.no_grad():
                ...
                # ENTITY RANKING para validación (early stopping)
                print("Validation (entity ranking):")
        ...
        if early_stopping_epoch is not None:
            if best_epoch is not None:
                print(f"\nEntrenamiento detenido por early stopping en la epoca {early_stopping_epoch}. El mejor modelo proviene de la epoca {best_epoch} (MRR = {best_val_mrr:.4f}).")
            else:
                print(f"\nEntrenamiento detenido por early stopping en la epoca {early_stopping_epoch}. No se registro una mejora en MRR para guardar un modelo.")
        else:
            if best_epoch is not None:
                print(f"\nEntrenamiento completado sin early stopping. El mejor modelo proviene de la epoca {best_epoch} (MRR = {best_val_mrr:.4f}).")
            else:
