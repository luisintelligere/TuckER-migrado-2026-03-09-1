...
class Experiment:
    # (El constructor __init__ y las funciones get_data_idxs, get_er_vocab, get_batch, evaluate no cambian)
    ...
    def evaluate(self, model, data):
        ...
        for i in range(0, len(test_data_idxs), self.batch_size):
            ...
            e1_idx = torch.tensor(data_batch[:,0]).to(self.device); r_idx = torch.tensor(data_batch[:,1]).to(self.device); e2_idx = torch.tensor(data_batch[:,2]).to(self.device)
    ...
    def train_and_eval(self, init_embeddings_path=None, init_vocab_path=None): # <-- Parámetros añadidos
        ...
        # --- INICIO DE LA MODIFICACIÓN PARA INYECTAR EMBEDDINGS ---
        if init_embeddings_path and init_vocab_path:
            print(f"🔹 Cargando embeddings iniciales desde {init_embeddings_path}")
            try:
                # Cargamos la matriz y el vocabulario pre-generados
                init_embeddings = torch.load(init_embeddings_path, map_location=self.device)
                
                # Inyectamos los embeddings en la nueva matriz del modelo
                with torch.no_grad():
                    model.E.weight.data.copy_(init_embeddings)
                print("✅ Embeddings iniciales inyectados en el modelo.")
            except FileNotFoundError:
                print(f"⚠️ Advertencia: No se encontró el archivo de embeddings. Usando inicialización aleatoria.")
                model.init()
        else:
            # Si no se proveen embeddings, se usa la inicialización aleatoria original
            model.init()
...
if __name__ == '__main__':
    ...
    experiment = Experiment(
        ...
        output_dir=output_dir # Pasamos el output_dir calculado
        )
    
    experiment.train_and_eval(init_embeddings_path=args.init_embeddings)