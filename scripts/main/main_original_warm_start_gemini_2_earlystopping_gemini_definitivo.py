...
# Versión del codigo que ocupa una función de evaluación que rankea las relaciones sin incluir las *_reverse
...
class Experiment:
    ...
    def evaluate(self, model, data, rank_relations=False):
        """
        Si rank_relations=False  -> evalúa rankeando ENTIDADES (como siempre).
        Si rank_relations=True   -> evalúa rankeando RELACIONES (filtrando _reverse).
        Devuelve el mismo dict de métricas: hits@10, hits@3, hits@1, mr, mrr.
        """
        import numpy as np
        model.eval()

        # Índices de (h,r,t) según los mapas de ESTA clase
        test_data_idxs = self.get_data_idxs(data)          # [(h_idx, r_idx, t_idx), ...]
        if not test_data_idxs:
            return {'hits@10': np.nan, 'hits@3': np.nan, 'hits@1': np.nan, 'mr': np.nan, 'mrr': np.nan}

        if not rank_relations:
            # ================= ENTIDADES (código original) =================
            hits = [[] for _ in range(10)]
            ranks = []

            # "Filtered setting" para entidades
            ...
            for i in range(0, len(test_data_idxs), self.batch_size):
                ...
                # filtrar otras verdaderas excepto el gold
                for j in range(batch.shape[0]):
                    filt = er_vocab[(batch[j][0], batch[j][1])]
                    target_value = predictions[j, e2_idx[j]].item()
                    if filt:
                        predictions[j, filt] = 0.0
                    predictions[j, e2_idx[j]] = target_value

                sort_idxs = torch.argsort(predictions, dim=1, descending=True).cpu().numpy()
                for j in range(batch.shape[0]):
                    rank = int(np.where(sort_idxs[j] == e2_idx[j].item())[0][0]) + 1
                    ranks.append(rank)
                    for k in range(10):
        ...
        # ================= RELACIONES (parche nuevo) =================
        # 1) Prepara lista de relaciones FORWARD (sin _reverse)
        rel_names_fwd = [r for r in d.relations if not r.endswith("_reverse")]
        rel_list_fwd  = [self.relation_idxs[r] for r in rel_names_fwd if r in self.relation_idxs]

        hits = [[] for _ in range(10)]
        ranks = []

        # Para contar sólo ejemplos "forward" en el print
        n_forward = sum(1 for (_, r, _) in data if not r.endswith("_reverse"))
        print("Number of data points:", n_forward)

        # Recorremos en paralelo: índices y tripletas crudas (para saber si son _reverse)
        for (h_idx, r_idx, t_idx), (h_raw, r_raw, t_raw) in zip(test_data_idxs, data):
            # Saltar las tripletas con relación _reverse (solo evaluamos forward)
            if r_raw.endswith("_reverse"):
                continue

            e_h = model.E.weight[h_idx]
            e_t = model.E.weight[t_idx]

            # 2) Score para CADA relación forward en el mismo orden de rel_names_fwd
            scores = []
            for ridx in rel_list_fwd:
                e_r = model.R.weight[ridx]
                # Detectar layout del núcleo W y contraer con r
                if model.W.shape[0] == e_r.shape[0]:
                    M_r = torch.tensordot(model.W, e_r, dims=([0], [0]))   # [d_e, d_e]
                else:
                    M_r = torch.tensordot(model.W, e_r, dims=([1], [0]))   # [d_e, d_e]
                s = (e_h @ M_r @ e_t).item()
                scores.append(s)

            scores = np.asarray(scores, dtype=float)  # alineado con rel_names_fwd

            # 3) Relación verdadera (forward)
            gold_name = r_raw.replace("_reverse", "")
            if gold_name not in rel_names_fwd:
                # si no está (raro), saltamos este caso
                continue
            gold_pos = rel_names_fwd.index(gold_name)

            # 4) Ranking del índice gold_pos dentro de los scores
    ...
    def train_and_eval(self, init_embeddings_path=None, init_vocab_path=None):
        ...
        for it in range(1, self.num_iterations + 1):
            ...
            with torch.no_grad():
                ...
                print("Validation:")
                val_metrics = self.evaluate(model, d.valid_data, rank_relations=True)
