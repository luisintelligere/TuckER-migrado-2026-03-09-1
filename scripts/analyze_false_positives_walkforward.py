...
from tuning_temporal_walkforward import (
    TEST_GEN,
    TRAIN_GENS,
    VALID_GEN,
    build_model,
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description=(
            "Reconstruye los mejores modelos del tuning walk-forward y genera "
    ...
    parser.add_argument(
        "--selection-file",
        type=Path,
        default=RESULTS_DIR / "tuning_walkforward_mejores_validacion.csv",
        help="CSV con los modelos seleccionados que se desea reconstruir.",
    )
    parser.add_argument(
        "--selection-label",
        type=str,
        default="mejor_validacion_2022",
        help="Etiqueta descriptiva del criterio de seleccion.",
    )
    parser.add_argument(
        "--output-fp",
        type=Path,
        default=RESULTS_DIR / "tuning_walkforward_falsos_positivos_validacion.csv",
        help="CSV detalle de falsos positivos.",
    )
    parser.add_argument(
        "--output-summary",
        type=Path,
        default=RESULTS_DIR / "tuning_walkforward_falsos_positivos_resumen_validacion.csv",
        help="CSV resumen de falsos positivos por curso.",
    )
...
def analyze_false_positives(
    ...
    all_relevant_courses = CURSOS_S1 + CURSOS_S2
    ...
    for alumno_id in fp_ids:
        ...
        for curso in CURSOS_S1 + CURSOS_S2:
        ...
        registro_s3 = first_course_record(historial, curso_target)
        if registro_s3 is None:
            row["nota_real_s3"] = np.nan
            row["estado_real_s3"] = "Sin registro"
        else:
            row["nota_real_s3"] = registro_s3["NOTA"]
            row["estado_real_s3"] = registro_s3["ESTADO_CURSO"]

        notas_s1 = [row.get(f"{curso}_nota") for curso in CURSOS_S1 if pd.notna(row.get(f"{curso}_nota"))]
        notas_s2 = [row.get(f"{curso}_nota") for curso in CURSOS_S2 if pd.notna(row.get(f"{curso}_nota"))]
        notas_all = [nota for nota in notas_s1 + notas_s2 if isinstance(nota, (int, float))]

        row["promedio_s1"] = float(np.mean(notas_s1)) if notas_s1 else np.nan
        row["promedio_s2"] = float(np.mean(notas_s2)) if notas_s2 else np.nan
        row["promedio_general"] = float(np.mean(notas_all)) if notas_all else np.nan

        reprobados_previos = sum(
            1
            for curso in all_relevant_courses
            if "Reprobado" in str(row.get(f"{curso}_estado", ""))
        )
        sin_registro = sum(
            1 for curso in all_relevant_courses if row.get(f"{curso}_estado") == "Sin registro"
        )
        row["cursos_reprobados_previos"] = reprobados_previos
        row["cursos_sin_registro"] = sin_registro
        row["vulnerable"] = "Si" if (reprobados_previos >= 2 or (notas_all and np.mean(notas_all) < 4.5)) else "No"
...
def build_summary_row(
    ...
    if fp_df.empty:
        return {
            ...
            "nota_real_media_s3": np.nan,
            "promedio_general_s1_s2": np.nan,
            "reprobados_previos_promedio": np.nan,
        }
    ...
    return {
        ...
        "nota_real_media_s3": round(float(fp_df["nota_real_s3"].mean()), 3),
        "promedio_general_s1_s2": round(float(fp_df["promedio_general"].mean()), 3),
        "reprobados_previos_promedio": round(float(fp_df["cursos_reprobados_previos"].mean()), 3),
    }
